	...
	<div class="htmleaf-container">
		<header class="htmleaf-header">
			...
			<div class="htmleaf-links">
				...
				<a class="htmleaf-icon icon-htmleaf-arrow-forward-outline" href="user.jsp?uid=<%=uid %>" title="返回管理员用户界面" target="_blank"><span> 返回下载页</span></a>
			</div>
		</header>
		<div class="container">
			<div class="row">
				<div class="col-md-12" style="padding:2em 0;">
				  <div class="table-responsive">
						<table class="table table-bordered table-striped" id="mytable">
						    <thead>
<tr><td>歌曲名</td><td>类别id</td><td>类别</td><td>操作</td></tr>
						    </thead>
						    <tbody>
<%
	//String uid=request.getParameter("uid");
...
<tr><td><%=m.getMname() %></td><td><%=m.getKid()%></td><td><%=m.getKname()%></td>

   <td>

       
       <a href="playmusics?uid=<%=u.getUid() %>&kid=<%=m.getKid() %>"><button class="btn btn-info" id="add"><i class="fa fa-plus"></i> 播放</button></a>
       <a href="lmusicadd?uid=<%=u.getUid() %>&mname=<%=m.getMname() %> &kid=<%=m.getKid() %>"><button class="btn btn-info" id="add"><i class="fa fa-plus"></i> 喜欢</button></a>
